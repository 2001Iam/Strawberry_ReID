...
from model import ft_net, ft_net34, ft_net18, ft_net_dense, ft_net_hr, ft_net_swin, ft_net_swinv2, ft_net_efficient, \
    ft_net_NAS, ft_net_convnext, PCB, PCB_test
from utils import fuse_all_conv_bn
from dataloader.dataloader_berry import BerryDatasets
...
parser.add_argument('--use_resnet18', action='store_true', help='use resnet18')
parser.add_argument('--use_resnet34', action='store_true', help='use resnet34')
parser.add_argument('--PCB', action='store_true', help='use PCB')
parser.add_argument('--multi', default=True, action='store_true', help='use multiple query')
...
if 'use_resnet18' in config:
    opt.use_resnet18 = config['use_resnet18']
if 'use_resnet34' in config:
    opt.use_resnet34 = config['use_resnet34']
...
if opt.multi:
    image_datasets = {x: BerryDatasets(os.path.join(data_dir, x), data_transforms) for x in
                      ['gallery', 'query', 'multi-query']}
    dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=opt.batchsize,
                                                  shuffle=False, num_workers=16) for x in
                   ['gallery', 'query', 'multi-query']}
else:
    image_datasets = {x: BerryDatasets(os.path.join(data_dir, x), data_transforms) for x in ['gallery', 'query']}
    dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=opt.batchsize,
...
gallery_cam = [1] * len(gallery_cam)

print(gallery_cam)
print(gallery_label)
query_cam, query_label = get_id(query_path)
query_cam = [0] * len(query_cam)
...
elif opt.use_resnet18:
    model_structure = ft_net18(opt.nclasses, stride=opt.stride, linear_num=opt.linear_num)
elif opt.use_resnet34:
    model_structure = ft_net34(opt.nclasses, stride=opt.stride, linear_num=opt.linear_num)
else:
