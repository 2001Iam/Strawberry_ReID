...
from model import ft_net, ft_net18, ft_net34, ft_net_dense, ft_net_hr, ft_net_swin, ft_net_swinv2, ft_net_convnext, \
    ft_net_efficient, ft_net_NAS, PCB
...
if opt.use_dense:
    ...
    model = ft_net18(len(class_names), opt.droprate, opt.stride, circle=return_feature, linear_num=opt.linear_num)
elif opt.use_resnet34:
    model = ft_net34(len(class_names), opt.droprate, opt.stride, circle=return_feature, linear_num=opt.linear_num)
...
if torch.cuda.get_device_capability()[0] > 6 and len(opt.gpu_ids) == 1 and int(
        version[0]) > 1:  # should be >=7 and one gpu
